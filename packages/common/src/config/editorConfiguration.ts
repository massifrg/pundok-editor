...
const CONFIG_KEYS_PRECEDENCE: Record<string, number> = Object.fromEntries([
  'name', // string;
  'version', // : number[];
  'description', // : string;
  'inherits', // ?: string[];
...
function compareConfigKeys(context?: string): (k1: string, k2: string, context?: string) => number {
  return (k1, k2) => {
    ...
    const precedence: Record<string, number> = CONFIG_KEYS_PRECEDENCE
...
function sortConfigKeys(value: any, context?: string): any {
  if (Array.isArray(value)) {
    return value.every(v => isObject(v) && (v as NamedObject).name)
      ? value.sort((o1: NamedObject, o2: NamedObject) => o1.name.localeCompare(o2.name))
      : value
...
export function serializeConfiguration(config: PundokEditorConfig | PundokEditorConfigInit): string {
  const sortedObj = sortConfigKeys(config);
